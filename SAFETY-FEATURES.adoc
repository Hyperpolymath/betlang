...
BetLang is the first probabilistic programming language with
comprehensive gambling harm reduction and mathematical safety
guarantees.
...
Prevents arbitrage opportunities by validating that probabilities sum to
1.0 within tolerance.

*Features:* - Compile-time probability validation - Dutch book detection
from betting odds - Probability normalization - Bookmaker margin
calculation - Validated bet execution (`+bet/validated+`)
...
Prevents catastrophic loss through Kelly criterion and Monte Carlo
simulation.

*Features:* - Kelly criterion for optimal bet sizing - Risk-of-ruin
probability calculation - Stake safety validation - Wealth trajectory
simulation - Maximum drawdown analysis - Value-at-Risk (VaR) integration
...
*Features:* - Mandatory delays between bets - Violation tracking and
alerting - Session statistics (bets per minute, duration) - Adaptive
cool-off (increases with rapid betting) - Self-exclusion support
...
|DistnumberNormal |Everyday probabilistic arithmetic |High |Medium |Easy
(FFI)
...
|ImpreciseProbability |Interval-valued bounds |Medium |High |Moderate
(FFI)
...
[arabic]
. *Dutch Book Theorem* - Frank Ramsey (1926), Bruno de Finetti (1937)
. *Kelly Criterion* - John Larry Kelly Jr. (1956)
. *Risk-of-Ruin* - Gambler’s Ruin Problem, classic probability theory
. *Responsible Gambling* - GamCare, National Council on Problem Gambling
