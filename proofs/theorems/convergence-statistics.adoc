...
This document provides rigorous proofs for the statistical properties of
betlang, including limit theorems, convergence rates, error bounds, and
the theoretical foundations of the statistical inference library.
...
*Theorem 1.1* (WLLN for Ternary Bets). Let X₁, X₂, …, Xₙ be i.i.d.
samples from (bet A B C) where A, B, C ∈ ℝ. Then:
...
_Proof_: By Chebyshev’s inequality:
...
As n → ∞, this bound → 0, so latexmath:[\bar{X}_n \xrightarrow{P} \mu].
∎
...
_Proof_: Since E[X₁⁴] < ∞ (finite fourth moment for bounded discrete
distribution), Kolmogorov’s SLLN applies directly. ∎

==== 1.3 Weighted Bet Extension

*Theorem 1.3* (SLLN for Weighted Bets). For weighted bet with
...
_Proof_: Same as Theorem 1.2 with modified expectation. ∎
...
*Theorem 2.1* (CLT for Ternary Bets). For i.i.d. samples from (bet A B
C):
...
_Proof_: The moment generating function of Xᵢ exists and is finite
(bounded support). By Lévy’s CLT, the standardized sum converges to
standard normal. ∎
...
_Proof_: For ternary bets, the third absolute moment is bounded:
...
*Corollary 2.1* (Practical Bound). For uniform (bet A B C) with A, B, C
∈ [-1, 1]:
...
*Theorem 3.1* (Frequency Convergence). The `+bet-probability+`
estimator:
...
*Corollary 3.1* (Sample Size for Precision). To achieve margin of error
ε:
...
For ε = 0.01 and 95% confidence: n ≥ 9604 (using p = 0.5
conservatively).
...
_Proof_: Direct application of Hoeffding’s inequality for bounded random
variables in [0,1]. ∎
...
_Proof_: By Taylor expansion of -p log p around the true probability. ∎

*Corollary 4.1* (Bias for Ternary Bet). For uniform ternary bet (|S| =
3):
...
_Proof_: Using the delta method on the entropy functional. ∎
...
*Theorem 5.1* (Glivenko-Cantelli). The empirical CDF converges
uniformly:
...
_Proof_: Dvoretzky-Kiefer-Wolfowitz inequality. ∎
...
The `+kolmogorov-smirnov+` function uses this for goodness-of-fit
testing.
...
*Theorem 6.1* (MCMC Ergodicity). For an irreducible, aperiodic Markov
chain with stationary distribution π:
...
_Proof_: By the Markov chain ergodic theorem. ∎
...
*Theorem 6.3* (Geometric Convergence). If the MH chain is geometrically
ergodic:
...
_Proof_: SLLN for i.i.d. samples. ∎

==== 7.2 Variance Reduction

*Theorem 7.2* (Importance Sampling Variance). With importance
distribution q:
...
*Theorem 7.3* (Antithetic Variance Reduction). For antithetic pairs (X,
X’):
...
*Corollary 7.1* (Ternary Antithetic). For symmetric bet A, 0, -A with
f(x) = x:
...
the bootstrap distribution of √n(θ̂* - θ̂) converges to the true sampling
distribution of √n(θ̂ - θ) for smooth functionals θ.

_Proof_: Efron’s theorem on bootstrap consistency. ∎
...
The `+bootstrap+` function in lib/statistics.rkt provides consistent
variance estimates.
...
*Theorem 9.1* (Bernstein-von Mises). Under regularity conditions, the
posterior converges to a normal distribution:
...
*Theorem 9.2* (Doob’s Consistency). For "`most`" priors (in a
measure-theoretic sense), the posterior concentrates around the true
parameter:
...
*Theorem 9.3* (Frequentist Coverage of Bayesian CIs). For regular
problems:
...
as n → ∞. The Bayesian credible interval has asymptotically correct
...
*Theorem 10.1* (Random Walk Expectation). For `+(random-walk n)+` with
steps \{-1, 0, +1}:
...
_Proof_: - E[step] = (-1 + 0 + 1)/3 = 0 - E[step²] = (1 + 0 + 1)/3 = 2/3
- Var(step) = 2/3 - 0² = 2/3 - By independence, Var(Sₙ) = n · Var(step)
= 2n/3 ∎
...
*Theorem 10.3* (Expected Hitting Time). For hitting level k starting
from 0:
...
*Theorem 11.1* (Donsker’s Theorem). The rescaled random walk converges
to Brownian motion:
...
*Theorem 11.2* (Stable Law Convergence). Lévy flights with index α
converge to α-stable distributions:
...
*Theorem 12.1* (Sample Complexity for ε-δ Estimation). To estimate a
probability p within ε with confidence 1-δ:
...
*Theorem 12.2* (Bias-Variance-Computation). For time budget T and cost c
per sample:
...
*Theorem 13.1* (Floating-Point Error). For entropy calculation with n
samples:
...
For Racket’s random, P ≈ 2⁶¹, so this is not a practical concern for
...
[arabic]
. *Optimal importance sampling*: Derive optimal proposal for
betlang-specific computations
. *Adaptive MCMC convergence*: Prove convergence for adaptive MH in
lib/bayesian.rkt
. *Sequential Monte Carlo*: Prove particle filter convergence rate
. *Non-asymptotic Bayesian bounds*: Finite-sample posterior
concentration
...
[arabic]
. Durrett, R. (2019). _Probability: Theory and Examples_
. Van der Vaart, A.W. (1998). _Asymptotic Statistics_
. Robert, C.P. & Casella, G. (2004). _Monte Carlo Statistical Methods_
. Efron, B. & Tibshirani, R. (1993). _An Introduction to the Bootstrap_
. Meyn, S.P. & Tweedie, R.L. (2009). _Markov Chains and Stochastic
Stability_
