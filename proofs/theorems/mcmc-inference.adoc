...
This document provides rigorous proofs of correctness for the Markov
Chain Monte Carlo (MCMC) and inference methods implemented in betlang.
We establish convergence guarantees, prove detailed balance, and analyze
the statistical properties of the samplers.
...
*Definition 1.1* (Detailed Balance). A transition kernel P satisfies
detailed balance w.r.t. π if:
...
*Theorem 1.1* (MH Satisfies Detailed Balance). The MH transition kernel
satisfies detailed balance with target π.

_Proof_: The MH kernel is:
...
where α(x,y) = min(1, π(y)q(x|y)/(π(x)q(y|x))) and r(x) = 1 - ∫
q(y|x)α(x,y)dy.
...
*Theorem 1.2* (π is Invariant). If P satisfies detailed balance with π,
then π is invariant:
...
*Theorem 1.3* (MH Ergodicity). If the MH chain is irreducible and
aperiodic, then:
...
_Proof_: By the ergodic theorem for Markov chains with invariant
distribution. ∎
...
*Lemma 1.2* (Aperiodicity). If q(x|x) > 0 or there exist states with
positive probability of rejection, MH is aperiodic.
...
*Theorem 2.1* (Gibbs Preserves Joint). If p(x|y) and p(y|x) are the true
conditionals of joint p(x,y), then Gibbs sampling has p(x,y) as
invariant distribution.

_Proof_: The transition kernel is:
...
[latexmath]
++++
= \int p(y) p(x'|y) dy · p(y'|x')
++++
(integrating out x using ∫p(x|y)dx = 1)

[latexmath]
++++
= p(x') · p(y'|x')
++++
(using ∫p(x’|y)p(y)dy = p(x’))
...
*Theorem 2.2* (Geometric Convergence). Under regularity conditions,
Gibbs sampling converges geometrically:
...
*Theorem 3.1* (Volume Preservation). Leapfrog is symplectic (preserves
volume in phase space).

_Proof_: Each step is a shear transformation with unit Jacobian
determinant. Composition preserves this property. ∎

==== 3.4 Detailed Balance

*Theorem 3.2* (HMC Detailed Balance). HMC satisfies detailed balance
...
_Proof_: 1. Momentum flip: (q, p) ↔ (q, -p) is self-inverse 2. Leapfrog:
approximately preserves H(q,p) = U(q) + K(p) 3. MH correction: accounts
for numerical integration error ∎

==== 3.5 No-U-Turn Sampler (NUTS)

*Theorem 3.3* (NUTS Correctness). NUTS maintains detailed balance while
adaptively choosing path length.

_Proof sketch_: The doubling procedure and slice sampling maintain
reversibility. ∎
...
*Theorem 4.1* (Rejection Sampling Correctness). The output follows
distribution f.

_Proof_: P(accept x ∈ A) = P(X ∈ A, U < f(X)/(M·g(X)) | accept)
...
*Corollary 4.1* (Expected Samples). Expected number of proposals until
acceptance: M.
...
*Definition 5.1* (Importance Sampling Estimator). For target p, proposal
q:
...
*Theorem 5.3* (Optimal IS Proposal). The variance-minimizing proposal
is:
...
_Proof_: By Lagrange multipliers on variance subject to ∫q = 1. ∎
...
_Proof_: By sequential application of importance sampling with
...
*Theorem 6.2* (ESS Bound). 1 ≤ ESS ≤ N with equality at N iff uniform
weights.
...
_Proof_: As ε → 0, the acceptance region shrinks to \{D = D_obs},
recovering exact posterior. ∎
...
*Theorem 8.1* (Convergence Criterion). R̂ → 1 as n → ∞ for converged
chains.
...
*Theorem 8.2* (Geweke Test). Under convergence, for first 10% and last
50% of chain:
...
*Correctness*: Follows Theorem 1.1 (detailed balance) assuming symmetric
proposal.
...
*Theorem 11.2* (Burn-in Bound). To achieve TV distance ε from
stationarity:
...
[arabic]
. *Adaptive MCMC*: Prove ergodicity of adaptive algorithms
. *Parallel tempering*: Prove mixing time bounds
. *Gradient estimation*: Bias/variance of stochastic gradients
. *Variational inference*: Approximation bounds
...
. Brooks, S., et al. (2011). _Handbook of Markov Chain Monte Carlo_
. Doucet, A., de Freitas, N., & Gordon, N. (2001). _Sequential Monte
Carlo Methods in Practice_
. Neal, R.M. (2011). "`MCMC using Hamiltonian dynamics`"
. Marin, J.-M., et al. (2012). "`Approximate Bayesian computational
methods`"
