...
This document establishes the type-theoretic foundations of betlang,
formalizing the probability monad structure, proving the monad laws, and
establishing type safety. We present a gradual type system suitable for
a dynamically-typed host language (Racket) with optional static typing
guarantees.
...
*Definition 1.1* (Probability Monad). The triple (Dist, η, μ) forms a
monad where:
...
Equivalently, using Kleisli presentation with bind: - *return*: A →
Dist(A) - *>>=* (bind): Dist(A) → (A → Dist(B)) → Dist(B)
...
This is distributionally equivalent to m since bet-pure maps each
outcome back to itself. More formally:

Let m produce value v with probability P(v). Then: - bet-bind m bet-pure
produces (bet v v v) for each sampled v - (bet v v v) = v with
probability 1 - Therefore the output distribution equals the input
distribution ∎
...
_Proof_: By expansion using monad laws and function application. ∎
...
_Proof_: Follows from monad associativity. ∎
...
*Definition 4.1* (Kleisli Category for Dist). - *Objects*: Types A, B,
C, … - *Morphisms*: Kleisli arrows A ⇝ B = A → Dist(B) - *Identity*: η :
A → Dist(A) - *Composition*: (g ∘_K f) = λx. bind (f x) g
...
1. e is a value, or 2. ∃e’. e → e’ (e can take a step)

_Proof_: By induction on the typing derivation.

*Case* bet e₁ e₂ e₃: - By IH, each eᵢ either is a value or steps - If
all are values, the bet expression steps via E-BET - If any eᵢ steps,
the whole expression steps by congruence

*Case* application (e₁ e₂): - By IH on e₁ and e₂ - If e₁ = λx.e and e₂ =
v, step by β-reduction - Otherwise, step a subexpression
...
_Proof_: By induction on the evaluation derivation.

*Case* E-BET: (bet v₁ v₂ v₃) → vᵢ - By inversion, Γ ⊢ vᵢ : τ for all i -
Therefore Γ ⊢ vᵢ : τ ✓

*Case* β-reduction: ((λx.e) v) → e[x ↦ v] - By inversion, Γ ⊢ λx.e : τ₁
→ τ₂ and Γ ⊢ v : τ₁ - So Γ, x:τ₁ ⊢ e : τ₂ - By substitution lemma, Γ ⊢
e[x ↦ v] : τ₂ ✓
...
If ⊢ e : τ and e →* e’ where e’ is irreducible, then e’ is a value of
type τ.

_Proof_: By induction using Progress and Preservation. ∎
...
*Theorem 9.2* (Natural Transformation). bet-pure is a natural
transformation:
...
*TODO*: Full dependent type system requires more infrastructure: - Π
types for probability-indexed functions - Σ types for existential
probability bounds - Refinement types for probabilistic assertions
...
The set of distributions satisfies: 1. *Idempotence*: p·μ + (1-p)·μ = μ
2. *Skew-commutativity*: p·μ + (1-p)·ν = (1-p)·ν + p·μ 3.
*Skew-associativity*: Nesting of mixtures associates properly
...
. Heunen, C., Kammar, O., et al. (2017). "`A Convenient Category for
