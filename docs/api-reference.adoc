...
Selects one of three weighted choices where probabilities are
proportional to weights.

*Parameters:* - Three lists, each containing a value and its weight
...
*Parameters:* - `+predicate+`: Boolean expression - `+A+`, `+B+`, `+C+`:
Any Racket values
...
*Parameters:* - Three zero-argument functions (thunks)
...
*Parameters:* - `+n+`: Number of iterations - `+f+`: Function to apply
at each step - `+init+`: Initial value
...
Creates a function that randomly selects one of three functions to
apply.

*Parameters:* - `+f+`, `+g+`, `+h+`: Functions to compose
...
*Parameters:* - `+f+`: Function to apply - `+lst+`: List of values
...
*Parameters:* - `+f+`: Binary function - `+init+`: Initial accumulator -
...
*Parameters:* - `+pred+`: Predicate function - `+lst+`: List to filter
...
*Parameters:* - `+n+`: Number of trials - `+A+`, `+B+`, `+C+`: Values to
bet on
...
*Parameters:* - `+n+`: Number of repetitions - `+thunk+`: Zero-argument
function to call
...
*Parameters:* - Variable number of bet specifications
...
*Parameters:* - `+predicate+`: Function testing result - `+thunk+`:
Zero-argument function generating results
...
*Parameters:* - `+seed+`: Integer seed for random number generator -
...
*Parameters:* - `+n+`: Number of trials - `+predicate+`: Function
testing outcomes - `+A+`, `+B+`, `+C+`: Bet values
...
*Parameters:* - `+samples+`: List of sample values
...
*Parameters:* - `+n+`: Number of trials - `+f+`: Function to apply to
outcomes - `+A+`, `+B+`, `+C+`: Bet values
...
*Parameters:* - `+samples+`: Original sample - `+n+`: Number of
bootstrap samples - `+statistic+`: Function to compute on each resample
...
*Parameters:* - `+n+`: Number of steps - `+dt+`: Time increment
...
*Parameters:* - `+states+`: List of possible states - `+transitions+`:
Hash of transition probabilities - `+initial+`: Initial state
...
*Parameters:* - `+bet-fn+`: Zero-argument function to analyze - `+n+`:
Number of samples
...
`+bet/weighted+` requires exactly 3 weighted choices - Statistical
functions require non-empty lists - Probabilities must be in range [0,
1] - Markov chains require valid state transitions
...
synchronization primitives (`+semaphore+`, `+channel+`, etc.) for
concurrent access.
...
API Version: 2.0 Last Updated: 2025

'''''

For more information, see: - link:tutorial.md[Tutorial] - Learn by
example - link:semantics.md[Semantics] - Formal specifications -
link:../examples/[Examples] - Code examples
