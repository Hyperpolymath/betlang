...
High-performance Julia backend for the betlang probabilistic programming
language.

=== Status

*Phase:* Step 1 - Minimal Viable Backend ✅ *Completion:* 20% (basic bet
operations working)
...
Expected speedup vs Racket: - Monte Carlo (1M samples): *75x faster* -
Distribution sampling: *100x faster* - Statistical operations: *50x
faster*
...
[arabic]
. *Add to BetLang.jl*: Implement in `+src/BetLang.jl+`
. *Add tests*: Update `+test/runtests.jl+`
. *Update compiler*: Add translation in
`+compiler/betlang-to-julia.rkt+`
. *Run tests*: `+just test+`
...
* Hyperpolymath integration:
`+../docs/hyperpolymath-julia-integration.md+`
